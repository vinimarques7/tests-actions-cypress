...
### Passo 0: Setup do Ambiente (Windows + WSL 2)

O WSL 2 (Windows Subsystem for Linux) permite rodar um ambiente Linux completo dentro do Windows. É o primeiro passo porque garante que seu ambiente de desenvolvimento seja idêntico ao servidor de produção.

**No PowerShell como Administrador:**
```powershell
wsl --install -d Ubuntu
```
> ⚠️ Reinicie o PC após este comando para ativar a virtualização.

**No terminal Ubuntu, instale o Node.js:**
```bash
# Atualiza os pacotes do sistema
sudo apt-get update

# Instala o Node.js via repositório oficial
...
### Passo 1: Instalação da Docker Engine no WSL 2

Instalamos a Docker Engine pura via terminal para evitar a sobrecarga do Docker Desktop. Execute os comandos abaixo linha por linha:

```bash
# Atualiza os pacotes e instala certificados de segurança
sudo apt-get update
sudo apt-get install -y ca-certificates curl

# Cria o diretório para as chaves de autenticação
...
# Adiciona seu usuário ao grupo docker (evita usar sudo em todo comando)
...
> **Por que não usar Docker Desktop?** O Docker Desktop tem uma camada de virtualização extra no Windows que pode causar lentidão e conflitos com o WSL 2. A Engine pura roda diretamente no Linux, mais rápida e estável.
